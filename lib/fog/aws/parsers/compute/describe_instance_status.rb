    ...
    module AWS
      module Compute
        class DescribeInstanceStatus < Fog::Parsers::Base
          def new_instance!
            @instance = { 'instanceState' => {}, 'systemStatus' => { 'details' => [] }, 'instanceStatus' => { 'details' => [] }, 'eventsSet' => [], 'attachedEbsStatus' => { 'details' => [] } }
          end
          ...
          def end_element(name)
            ...
            when 'nextToken', 'requestId'
              @response[name] = value
            when 'systemStatus', 'instanceState', 'instanceStatus', 'eventsSet', 'attachedEbsStatus'
              @inside = nil
            when 'item'
              case @inside
              when :eventsSet
